...
def evaluate(train, train_labels, test, test_labels):
    ...
    cv_score = 1 * cross_val_score(model, train, train_labels, scoring="f1", cv=5)
    ...
    print(
        "5-fold CV F1: {:.2f} with std: {:.2f}".format(cv_score.mean(), cv_score.std())
    )
...
def plot_feature_importances(df, n=15, threshold=None):
    ...
    if threshold:
        ...
        plt.vlines(
            importance_index + 1, ymin=0, ymax=1.05, linestyles="--", colors="red"
        )
        plt.show()
...
def feature_selection(feature_matrix, missing_threshold=90, correlation_threshold=0.95):
    ...
    feature_matrix = feature_matrix[
        [x for x in feature_matrix if x not in missing_cols]
    ]
    print(
        "{} missing columns with threshold: {}.".format(
            n_missing_cols, missing_threshold
        )
    )

    # Zero variance
    unique_counts = pd.DataFrame(feature_matrix.nunique()).sort_values(
        0, ascending=True
    )
    ...
    feature_matrix = feature_matrix[
        [x for x in feature_matrix if x not in zero_variance_cols]
    ]
    ...
    print(
        "{} collinear columns removed with threshold: {}.".format(
